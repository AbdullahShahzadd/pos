...
app.post("/addItem", ensureLogin, async (req, res) => {
...
app.post("/addLocation", ensureLogin, async (req, res) => {
	var user = await insertLocationMod.addLocation(req.body, req.session.user.id);
	req.session.user.locations = user.locations;
	res.redirect("testAddLocation")
})

app.post("/addCustomer", ensureLogin, async (req, res) => {
